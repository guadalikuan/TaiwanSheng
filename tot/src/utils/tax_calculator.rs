...
//! 
//! ============================================
// 文件: src/utils/tax_calculator.rs
// 动态税率计算器
...
impl TaxCalculator {
    ...
    pub fn calculate_tax(
        ...
        // 
        // 注意:
        // - 折扣会减少税率（使用saturating_sub防止下溢，这是预期的行为）
        // - 附加税会增加税率（使用checked_add检测溢出，防止异常）
        // - 最终税率不能为负数（最小为0）
        let after_discount = base_tax_bps.saturating_sub(holding_discount_bps);
        
        // 验证附加税不会导致溢出
        // 如果附加税导致税率超过u16最大值，返回错误
        let final_tax_bps = after_discount
            .checked_add(whale_tax_bps)
            .ok_or(error!(TotError::TaxRateExceedsMaximum))?;
...
impl TaxDistribution {
    ...
    pub fn calculate(total_tax: u64) -> Result<Self> {
        ...
        // 允许最多1个代币的舍入误差（由于整数除法）
        let diff = if sum > total_tax {
            sum - total_tax
        } else {
            total_tax - sum
        };
        ...
        require!(
            diff <= 1,
            TotError::TaxCalculationOverflow
        );
