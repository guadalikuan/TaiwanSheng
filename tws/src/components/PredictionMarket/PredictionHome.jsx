...
const HOUSE_WALLET_PUBKEY = new PublicKey(TWSCoin_MINT); // Temporary placeholder using Mint address
...
const PredictionHome = () => {
  ...
  // Data Source (Simulated Database)
  const [allData] = useState(PREDICTION_MARKETS);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setMarkets(prev => {
        ...
        const nextBatch = allData.slice(currentLen, currentLen + 5);
        
        if (currentLen + nextBatch.length >= allData.length) {
          setHasMore(false);
        }
  ...
  const fetchBalance = async (publicKeyStr) => {
    ...
    try {
      ...
      if (e.message && e.message.includes("could not find mint")) {
         // console.warn("TWSCoin mint not found on Devnet. Balance set to 0.");
         setBalance(0);
      } else {
  ...
  const executeBet = async () => {
    if (!confirmBet || !connected || !publicKey) return;
  ...
  const handleSaveMarket = (e) => {
    ...
    if (editingMarket) {
        // Edit Mode
        setMarkets(prev => prev.map(m => m.id === editingMarket.id ? { ...m, ...newMarketForm } : m));
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center overflow-hidden relative font-sans">
      ...
      <div className="w-full max-w-md p-4 flex justify-between items-center z-10 bg-gradient-to-b from-black to-transparent gap-4">
        ...
        <button onClick={connected ? logout : handleConnectClick} className={`px-3 py-1 text-sm rounded-sm font-bold border ${connected && publicKey ? 'bg-green-900/50 border-green-500 text-green-400' : 'bg-red-900/20 border-red-500 text-red-500 animate-pulse'}`}>
      ...
      {/* Carousel Area */}
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md relative z-10 px-4 mt-16">
        
        {/* History Button - Centered and Prominent */}
        <div className="w-full flex justify-center mb-8">
          <button 
            onClick={() => navigate('/predict/history')}
            className="px-8 py-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-black text-xl rounded-full shadow-[0_0_25px_rgba(147,51,234,0.7)] hover:shadow-[0_0_35px_rgba(147,51,234,0.9)] transition-all duration-300 transform hover:scale-105 border-2 border-purple-400/50"
          >
            BETTING HISTORY
          </button>
        </div>
