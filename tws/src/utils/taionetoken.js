...
// TaiOneToken 铸造地址（也是 TaiOne 财库地址）
// 优先使用环境变量 VITE_TWS_COIN_MINT（保持向后兼容）
...
export const TaiOneToken_MINT = import.meta.env.VITE_TWS_COIN_MINT || (
  import.meta.env.MODE === 'production' 
    ? DEFAULT_MAINNET_MINT 
    : DEFAULT_DEVNET_MINT
);

// TaiOneToken Decimals
export const TAI_ONE_DECIMALS = 6;

// 调试日志，帮助开发者确认当前使用的 Mint 地址和环境
console.log(`[TaiOneToken] Environment: ${import.meta.env.MODE}`);
console.log(`[TaiOneToken] Active Mint Address: ${TaiOneToken_MINT}`);
console.log(`[TaiOneToken] Decimals: ${TAI_ONE_DECIMALS}`);
 ...
 */
export const formatTaiOneTokenBalance = (balance, decimals = TAI_ONE_DECIMALS) => {
...
/**
 * 将 TaiOneToken 转换为最小单位
 * @param {number} amount - 数量
 * @param {number} decimals - 小数位数，默认 9
 * @returns {string} 最小单位数量
 */
export const toTaiOneTokenUnits = (amount, decimals = 9) => {
