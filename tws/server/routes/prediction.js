...
const initPredictionData = async () => {
  try {
    ...
    if (existingMarkets.length === 0) {
      const MARKETS_FILE = path.join(
        __dirname,
        "../data/predictionMarkets.json",
      );
      const MARKETS_BAK_FILE = path.join(
        __dirname,
        "../data/predictionMarkets.json.bak",
      );
      if (fs.existsSync(MARKETS_FILE)) {
        ...
        console.log(
          `[Migration] Migrated ${markets.length} prediction markets to RocksDB`,
        );
      }
    }
    ...
    if (existingBets.length === 0) {
      ...
      const DATA_BAK_FILE = path.join(
        __dirname,
        "../data/predictionBets.json.bak",
      );
      if (fs.existsSync(DATA_FILE)) {
        ...
        console.log(
          `[Migration] Migrated ${bets.length} prediction bets to RocksDB`,
        );
      }
    }
  } catch (error) {
...
router.post("/bet", async (req, res) => {
  try {
    const { walletAddress, marketId, direction, amount, signature, timestamp } =
      req.body;
...
router.post("/verify-bet", async (req, res) => {
  try {
    const { txSignature, walletAddress, amount, marketId, direction } =
      req.body;
...
router.post("/distribute", checkAdminPermission, async (req, res) => {
  try {
    ...
    if (!marketId || !winningOutcome) {
      return res.status(400).json({
        success: false,
        message: "Missing marketId or winningOutcome",
      });
    }

    // 1. Get all bets for this market
    const allBets = await getBets();
    const marketBets = allBets.filter(
      (b) => b.marketId == marketId && b.status === "PENDING",
    );

    if (marketBets.length === 0) {
      return res.json({
        success: false,
        message: "No pending bets found for this market.",
      });
    }

    // 2. Calculate pools
    const poolYes = marketBets
      .filter((b) => b.direction === "YES")
      .reduce((sum, b) => sum + Number(b.amount), 0);
    const poolNo = marketBets
    ...
    if (winningBets.length === 0) {
      ...
      return res.json({
        success: true,
        message: "No winners. Pool kept in treasury.",
        distributions: [],
      });
    }
    ...
    console.log(
      `Distributing ${distributablePool} TOT to ${winningBets.length} winners + ${feeAmount} fee to Treasury`,
    );
    ...
    if (feeAmount > 0) {
      try {
        ...
        console.log(
          `✅ 平台费用已转给财库: ${feeAmount} TOT, Tx: ${feeResult.txHash}`,
        );
      } catch (error) {
      ...
      if (b.marketId == marketId && b.status === "PENDING") {
        if (b.direction === winningOutcome) {
          ...
          const result = results.find(
            (r) => r.wallet === b.walletAddress && r.betId === b.id,
          );
