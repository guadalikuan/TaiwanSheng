...
router.post(
  "/submit",
  authenticate,
  requireRole(ROLES.SUBMITTER, ROLES.ADMIN),
  async (req, res) => {
    try {
      const {
        ownerName,
      ...
      const rawAsset = {
        ...
        timestamp: Date.now(),
        submittedBy:
          req.user?.address || req.user?.username || req.user?.id || "unknown", // 保存提交者信息
      };
...
router.get(
  "/pending",
  authenticate,
  requireRole(ROLES.REVIEWER, ROLES.ADMIN),
  async (req, res) => {
    try {
...
router.get("/assets/:id", async (req, res) => {
  try {
    ...
    if (raw && raw.city) {
      ...
      const rawAsset = allAssets.raw.find(
        (r) => r.id === sanitized.internalRef,
      );
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(
          `✅ 资产已添加到资产池: ${assetData.sanitized?.codeName || id}`,
        );
      } catch (logError) {
      ...
      if (autoMint) {
        try {
          const toAddress =
            mintToAddress || req.user?.address || process.env.PLATFORM_WALLET;
          if (toAddress) {
            ...
            try {
              ...
              if (process.env.CONTRACT_ADDRESS) {
                console.warn(
                  "⚠️ Solana上链失败，尝试使用以太坊合约:",
                  solanaError.message,
                );
                try {
                  mintResult = await blockchainService.mintAsset(
                    assetData,
                    toAddress,
                  );
...
router.post(
  ...
  authenticate,
  requireRole(ROLES.REVIEWER, ROLES.ADMIN, ROLES.SUBMITTER),
  async (req, res) => {
    try {
      const { id } = req.params;
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const userAddress =
        req.user?.address || req.user?.username || req.user?.id;

      // 检查资产是否由当前用户提交
      if (
        raw.submittedBy &&
        raw.submittedBy !== userAddress &&
        req.user?.role !== "ADMIN"
      ) {
      ...
      try {
        ...
        if (homepageData?.map?.mainland?.logs) {
          homepageData.map.mainland.logs =
            homepageData.map.mainland.logs.filter((log) => log.assetId !== id);
          // 更新资产池统计
          if (homepageData.map.mainland.assetPoolValue && raw.debtAmount) {
            homepageData.map.mainland.assetPoolValue = Math.max(
              0,
