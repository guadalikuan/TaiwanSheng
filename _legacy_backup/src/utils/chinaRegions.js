...
export const PROVINCES_AND_CITIES = {
  '陕西省': {
    cities: ['西安', '咸阳', '宝鸡', '渭南', '汉中', '安康', '商洛', '延安', '榆林', '铜川']
  },
  '北京市': {
  ...
  '新疆维吾尔自治区': {
    cities: ['乌鲁木齐', '克拉玛依', '吐鲁番', '哈密', '昌吉回族自治州', '博尔塔拉蒙古自治州', '巴音郭楞蒙古自治州', '阿克苏地区', '克孜勒苏柯尔克孜自治州', '喀什地区', '和田地区', '伊犁哈萨克自治州', '塔城地区', '阿勒泰地区', '石河子', '阿拉尔', '图木舒克', '五家渠', '北屯', '铁门关', '双河', '可克达拉', '昆玉', '胡杨河', '新星', '白杨', '铁门关市']
  }
};
...
export const getProvinceList = () => {
  return Object.keys(PROVINCES_AND_CITIES);
};
...
export const getCitiesByProvince = (province) => {
  if (!province || !PROVINCES_AND_CITIES[province]) {
    return [];
  }
  return PROVINCES_AND_CITIES[province].cities || [];
};
...
export const getProvinceByCity = (cityName) => {
  ...
  for (const [province, data] of Object.entries(PROVINCES_AND_CITIES)) {
